...
**CI: Trunk E2E phone rail timeouts** - Complete ✓ (shared mobile viewport / drawer waits; PlaceHeading #110/#113 unchanged)
...
**Yoink UI story 20: Inbox warm triage surface** - Complete ✓ (#98 — costume only: Inbox/Snoozed/Trash share semantic `--inbox-surface` (theme-native mix of background + muted) plus light Inbox heading + “N to process · references & triage”; Today/lists stay on `--background`; light/dark/tokyo-night; Promote verb, rail, and capture cards unchanged)
**Yoink UI story 21: dense capture snippets** - Complete ✓ (#99 — costume: Inbox/Snoozed/Trash capture rows read as dense clippings — content + `sourceApp` + relative time (`Chrome · just now` / `typed · 10m ago`), tighter chrome, no complete/grip/pin; Promote / Snooze / Trash unchanged; ~44px mobile targets; theme tokens in light/dark/tokyo-night)
**Yoink UI story 22: loud Inbox-vs-task rail** - Complete ✓ (#100 — costume only: Inbox is a capture/triage **mode** at the top of the rail (count badge still hidden at 0) with “Capture & triage mode” cue, then a token-colored separator + Task family heading before Today → Upcoming → Mine → Done; Lists heading + named lists + Unlisted + + New list stay. Same structure in the mobile Tasks drawer; thumb bar stays Inbox | Tasks. Light / dark / tokyo-night via `--inbox-surface` / `--border` / muted headings — no hard-coded colors. Destinations unchanged; All stays retired.)
**Yoink UI story 23: desktop quick-capture shortcut** - Complete ✓ (#101 — desktop `md+` ⌘K / Ctrl+K focuses the Inbox capture field, including from task screens; typed capture lands in Inbox. Chord swallows Chrome’s omnibox search. Does not steal typing from other inputs or dialogs. Mobile keeps the on-pane field with no shortcut chrome. Hint uses theme tokens in light / dark / tokyo-night. Promote / lifecycle unchanged.)
**Sentry: setPointerCapture NotFoundError on Tasks** - Complete ✓ (JHTC-YOINK-API-D — one-pile grip and Vaul/synthetic pointerdown no longer throw when the pointer is already gone; named-list / Unlisted reorder still persists)
**PWA update Refresh** - Complete ✓ (prompt banner stays; Refresh skipWaits the waiting worker + `clientsClaim` and reloads without a second `sw.js` fetch; hard-reload fallback; swallow SW update fetch failures; theme-token banner + Updating… busy state; Fixes JHTC-YOINK-API-2)
**PWA: don’t SW-fallback `/admin` navigations** - Complete ✓ (Workbox `navigateFallbackDenylist` for `/admin` and `/api`; installed PWA no longer serves the web Not Found for admin)
**Header org switcher (multi-org picker)** - Complete ✓ (chevron opens a body-portaled picker above header/rail/Vaul chrome; picking another org POSTs `/api/organizations/switch` and reloads; failure toasts instead of a silent `isSwitching` reset)
**Remove task pin from UI** - Complete ✓ (#111 — no pin/unpin chrome on task rows desktop + mobile; backend `pinnedAt` / pin-unpin API / store fields kept; one-pile `openOrder` and drag unchanged; smart views keep API order with no pin affordance)
**Yoink UI story 24: you-are-here headings** - Complete ✓ (#110 — Inbox-grade place chrome on Today / Upcoming / Mine / Done / named lists / Unlisted: view or list name as the primary heading plus a short count cue; Inbox heading language unchanged; mobile Inbox | Tasks thumb bar stays; Tasks landing stays Today; pin chrome stays gone)
**Yoink UI story 25: task-row hybrid chrome** - Complete ✓ (#115 — superseded by #118)
**Yoink UI story 26: tap-to-edit, reorder mode, drop-slot feedback** - Complete ✓ (#118 — normal rows are circle + title only; tap opens Edit; Delete lives in the dialog; named list / Unlisted Reorder mode for drag; lifted card + neighbor gap + primary insertion line; swipe-complete stays and does not open Edit)

Recent updates:
- #118 E2E: midpoint drop helpers cleared the 20 reorder timeouts (492/493). The leftover fail is the known phone-rail flake (`task-row-no-pin` “drawer opened but no rail item was on screen”) — retry the menu trigger when `aria-expanded` is still false; not a #118 product regression.
- #118 E2E drag helpers: PR 119 CI timed out waiting for `PUT …/tasks/order` because title-to-title Y sits above the frozen slot midpoint (`dropIndexForClientY`). One-step down-drags never changed order (no persist); Milk→Butter landed one slot short. Helpers now drop on the `data-sortable-item` mid (kit `moveOpenTask` uses the same path). Product lock and the 10 #118 cases unchanged.
- Yoink UI story 26 “tap-to-edit, reorder mode, drop-slot feedback” (#118): product lock Justin (2026-09-24, revised 2:40pm CT). #117 row chrome still felt crowded and drag had no landing-slot feedback; Edit through ⋯ was two taps. Normal mode on every task screen: complete circle + title only (no ⋯, pencil, trash, or grip). Tap anywhere except the circle opens the existing edit dialog; Delete moves inside that dialog (same confirm as today). Circle stays ~44px so near-misses resolve correctly; on Done the circle uncompletes and the row still opens Edit. Swipe-to-complete stays and must not also open Edit. Named list and Unlisted get a Reorder header button; smart views do not. Reorder mode: grip on the right, no Edit on tap, complete and swipe off, drag from anywhere, header reads “Reorder” with Done; Done or navigating away exits. While dragging: lifted shadowed card follows the pointer, neighbors slide apart into a real gap, a thick `bg-primary` insertion line (theme primary — shadcn `--accent` is a muted hover surface) sits in the gap and updates across a multi-slot drag (#90). Order saves on release. Pin stays gone. Theme tokens light/dark/tokyo-night. Phone and desktop. UI state only — no domain/API change. Supersedes #115/#117 chrome and #88’s explicit Edit button. Out of scope: Inbox↔Tasks unify, live cross-device updates, smart-view contents, new row actions.
- Yoink UI story 25 “task-row hybrid chrome” (#115): product lock Justin + Polly (2026-09-22). Rows were crowded — grip sat in the same cluster as Edit and Trash. One-pile screens (named list + Unlisted) keep the complete circle and a grip on the right; Edit and Delete move behind ⋯ (no pencil/trash icons next to the grip). Smart views (Today / Upcoming / Mine / Done) have no grip — reorder is already disallowed — and only ⋯ on the right. Complete circle stays; pin chrome stays gone (#111); swipe-to-complete stays; theme tokens in light / dark / tokyo-night. Out of scope: Inbox↔Tasks shell unify, realtime, pin revive, reorder rule changes. Superseded by #118.

- Trunk E2E stability (phone named-list / complete-control): Trunk #131 Quality/Docker/migrations passed; Deploy skipped because two Playwright cases timed out at 30s (`task-row-no-pin` phone named-list + Today; `task-row-complete-control` phone named-list). Not a PlaceHeading product regression (#110/#113) — both cases share mobile rail setup. Playwright `setViewportSize` can miss MediaQueryList `change`, so `useDesktopLayout` stayed desktop and the Tasks drawer trigger never mounted (`openMobileDrawer` then waited 30s). A closed Vaul drawer can also stay in the DOM; `count() > 0` was treated as “open,” so helpers clicked hidden rail items and `waitForURL` hung. Fix: re-read `matchMedia` on `resize`; settle phone/desktop chrome after viewport changes; treat only an on-screen rail item as an open drawer; retry rail destinations with a short URL wait; wait for the overlay to finish closing so complete/edit clicks are not intercepted. Timeouts stay 30s.
- Yoink UI story 24 “you-are-here headings” (#110): product lock Justin (2026-09-17). After Tasks on mobile, the pile was unlabeled — Inbox already had a clear heading. Every task view now reuses Inbox’s heading language (`text-2xl` title + muted cue): Today / Upcoming / Mine / Done / named list / Unlisted. Short cue is open count (assigned on Mine, completed on Done). Generic header “Tasks” label is gone so the place name is the you-are-here. Rail drawer trigger stays. Thumb bar stays Inbox | Tasks. Landing stays Today. Pin chrome stays gone (#111). Theme tokens in light / dark / tokyo-night. Out of scope: removing/redesigning the bottom bar, realtime, pin remove, new destinations.
- Remove task pin from UI (#111): with manual `openOrder` on one-pile screens, pin no longer re-partitions the list, so the control felt dead. Task rows drop pin/unpin chrome (desktop + mobile, named-list / Unlisted / smart views). Trailing chrome is grip (one-pile) + Edit + delete. Backend `pinnedAt`, `POST /api/tasks/:id/pin` and `/unpin`, and store fields stay for a later revival. Do not change `openOrder` / drag rules. Smart views still render the filter API order (no pin control). Out of scope: deleting DB columns or pin endpoints, pinned section, realtime, headings (#110), bottom bar.
- Header org switcher chevron did nothing for multi-org sessions (Justin after joining `Polly UX`, 2026-09-15). Kit `DropdownMenu` in `organization-switcher.tsx` (Header on Inbox and Tasks) is the same family as named-list ⋯ / snooze: z-50 portal clipped or buried by header overflow / desktop rail / leftover Vaul overlay. Picker now portals to `document.body` with `position:fixed` and z-index 100; switch still `POST /api/organizations/switch` + reload; errors toast via sonner. Playwright: two orgs, open from header chevron, pick the other org, session/UI sticks; intercepted failure shows a toast. Out of scope: PWA `/admin` SW denylist (#108), seeding Polly UX data, lifecycle / Promote.
- PWA service worker with `scope: '/'` was SPA-fallbacking `/admin` navigations to the web `index.html` (TanStack Not Found) even though production still serves admin at `/admin`. Workbox `navigateFallbackDenylist` now skips `/admin` and `/api` document navigations. Out of scope: admin app, org create, passkey/auth. After deploy, one Refresh / new SW may be required before the denylist takes effect.
- Yoink UI story 23 “desktop quick-capture shortcut” (#101): product lock Justin + Polly (costume frame 2026-09-14; **⌘K cue only**). Desktop (`md+`) global ⌘K / Ctrl+K focuses the existing Inbox quick-capture field — or navigates to Inbox first when you are on a task screen — so a typed capture lands in Inbox without hunting the field. Chrome’s tab omnibox uses the same chord; we `preventDefault` on keydown (same approach as GitHub / Linear / Notion) rather than picking a different chord. The field shows a theme-token hint (`border-border` / `bg-muted` / `text-muted-foreground`; ⌘K on Apple, Ctrl+K elsewhere). Shortcut does not yank focus from other inputs or open dialogs. Mobile: no listener, no hint, on-pane field and share/extension capture stay. Out of scope: Android share / Chrome extension, capture card density (#99), rail split (#100), renaming Promote, pane warm triage (#98).
- Yoink UI story 22 “loud Inbox-vs-task rail” (#100): product lock Justin + Polly (costume frame 2026-09-14; **rail** only). Inbox sits first as a mode (`bg-inbox-surface` chip + “Capture & triage mode”), then a loud `--border` separator and Task family heading before Today → Done. Lists heading (uppercase tracking, same chrome as Task family) + named lists + Unlisted + dashed + New list. Count badge still hides at 0. No new destinations. Desktop always-visible sidebar is the loud split; mobile Tasks drawer shares the structure; thumb bar stays Inbox | Tasks. Distinct in light, dark, and tokyo-night. Overflow delete, wrap labels, Today / Unlisted / + New list stay. Out of scope: pane surface (#98), capture-card density (#99), ⌘K (#101), lifecycle / Promote domain, new nav destinations.
- PWA “new version available → Refresh” felt slow, and rare unhandled `TypeError`s from failing to fetch `sw.js` (Sentry JHTC-YOINK-API-2). Keep `registerType: 'prompt'`. Refresh now posts `SKIP_WAITING` to the waiting worker (no extra `registration.update()` round-trip), Workbox `clientsClaim: true` so the new worker takes control immediately, then reload — with a short hard-reload fallback if activation does not claim. Periodic/visibility `update()` rejections are swallowed. Banner uses theme tokens (`bg-primary/10`) and an Updating… busy state. Out of scope: auto-update, costume stories #99/#100/#101, pointer-capture / drag.
- Sentry `setPointerCapture` NotFoundError on `/_authenticated/tasks` (JHTC-YOINK-API-D and siblings JHTC-YOINK-API-4…C): Chrome throws when `setPointerCapture` runs without an active pointer (Playwright synthetic `pointerdown` on the mobile rail, a drag whose pointer already ended). Guard the one-pile grip handle and wrap `Element.prototype.setPointerCapture` so Vaul’s drawer (same tasks chunk) cannot surface the same uncaught DOMException. Drag still starts and persists open order; swipe-to-complete axis is unchanged. Out of scope: PWA banner, costume #99–#101.
- Yoink UI story 21 “dense capture snippets” (#99): product lock Justin + Polly (costume frame 2026-09-14; **textual + info density** of capture cards). Capture rows on Inbox / Snoozed / Trash read as snippets to process — source + time prominent (`sourceApp` mapped: browser-extension → Chrome, android-share → Android, web → typed), multi-line content, tighter card chrome, still **no** complete / grip / pin / due. Actions stay Promote / Snooze / Trash (do not rename Promote). http(s) content links and `sourceUrl` stay. Swipe snooze/trash stays; content links do not fire those. Same density language on mobile and desktop; mobile action hit targets stay ~44px. Theme tokens (not light-only blues). Snooze options portal to `document.body` with viewport-clamped `position:fixed` (same idea as named-list overflow) so Tomorrow is not parked outside the viewport by the swipe card’s overflow box. Out of scope: pane surface (#98), rail Inbox-as-mode (#100), ⌘K (#101), lifecycle / Add task / backlinks / vault sync.
- Yoink UI story 20 “Inbox warm triage surface” (#98): product lock Justin + Polly (costume frame 2026-09-14; warmth is **secondary**; costume uses **theme tokens**). Costume only — Inbox capture pane reads as triage/reference, not a peer of Today. Inbox | Snoozed | Trash share semantic `--inbox-surface` (`color-mix` of `--background` + `--muted`, wired in default light/dark and tokyo-night) plus a light Inbox heading and subcopy “N to process · references & triage”. No cream hex. Task screens stay on `--background`. Distinct in light, dark, and tokyo-night. Capture actions (Promote / Snooze / Trash), count badge, and tabs still work. Promote verb unchanged. Out of scope: #99 capture-card density, #100 rail mode chrome, #101 ⌘K quick capture, Promote rename, Add task, backlinks, vault sync.
...
## Yoink UI story 3: the list is the pile on add-task - Complete ✓

**Goal**: On a named-list or Unlisted screen, add-task has no list picker. New tasks land on that pile. Smart views and All overview keep the picker.

**Product rules (locked, Justin + Polly 2026-09-03):**
- One story at a time. This story only. This is a miss from story 1, not a new idea.
- Named-list screen: no list dropdown on add-task. New tasks go on that named list (reuse existing one-pile create that already sets `listId`).
- Unlisted screen: no list dropdown on add-task. New tasks stay unlisted (omit `listId`).
- Smart views (Today / Upcoming / Mine / Done): keep the list picker as they have today.
- All’s pile dropdown stays until All dies (later story). Do not retire All. All overview still has its pile dropdown and create-task list picker as fallback.
- Create-list stays on + New list / All as already shipped.
- Stay on shadcn New York in `@yoink/ui-base`. HTTP still only maps. No new domain field.

**Out of scope (later stories — do not implement here):**
- Inbox pane / Promote sheet / Mine leftover (remove Mine dropdown)
- All retirement
- Mobile redesign
- Drag, empty groups, bulk actions
- Rail delete (already shipped)
...
**Implementation:**
- Hide `#create-task-list` when the current All view is a named pile or Unlisted (`allPile.kind === 'named' | 'unlisted'`). Rail named-list / Unlisted links already land on those one-pile screens.
- When hiding, create with that pile’s `listId` (or omit for unlisted). Smart views and All overview still use the picker.
- Playwright: named-list add-task has no picker and the new task lands on that list; Unlisted add-task has no picker and the new task stays unlisted; Today / Upcoming / Mine still show the picker; All overview still has pile dropdown + create-task picker; rail overflow delete and + New list still work. HTTP driver stubs the new browser operations.

**Deliverable:** On a list screen the list is the pile — add-task does not ask which list, and All remains the fallback.

---

## Yoink UI story 4: Inbox pane with Snoozed and Trash tabs - Complete ✓

**Goal**: The Inbox rail destination is a capture pane. Inbox is the home tab; Snoozed and Trash sit beside it. Not a list of tasks.

**Product rules (locked, Justin + Polly 2026-09-03):**
- One story at a time. This story only. Layout, not new domain.
- Inbox holds captures, not tasks. Snoozed and Trash are tabs on that pane, not rail items.
- Rail: one Inbox row. It stays highlighted on Inbox, Snoozed, and Trash.
- Tab order is Inbox | Snoozed | Trash (Inbox first). Keep `/`, `/snoozed`, `/trash` if they already deliver that.
- Capture rows: Promote / Snooze / Trash stay. No checkbox, no drag, no due date.
- Quick-add capture stays on the Inbox tab only.
- Existing capture workflows stay: snooze / unsnooze / trash / restore / empty-trash / permanent delete.
- Count badge already exists from story 1 (hidden at 0) — do not rebuild it or change count rules.
- Promote sheet is the next story; keep today’s Promote UI (existing Create Task modal).
- Stay on shadcn New York in `@yoink/ui-base`. HTTP still only maps. No new domain field or API.
- Mobile stays Inbox | Tasks bottom tabs. Do not redesign mobile.

**Out of scope (later stories — do not implement here):**
- Promote sheet (title + optional list, default Unlisted)
- Mine leftover (remove Mine dropdown)
- Retire All
- Mobile rail-inside-Tasks
- Drag, empty groups, bulk actions, suggested-list at promote
- Changing Inbox count rules
- Capture/task API or store changes

**Implementation:**
- Shared `InboxPaneTabs` on `/`, `/snoozed`, `/trash` in order Inbox | Snoozed | Trash. Rail Inbox already links to `/` and already treats those three paths as active.
- Playwright: rail Inbox lands on the capture Inbox tab; count hidden at 0 and shown for unsnoozed inbox captures; Snoozed/Trash are pane tabs, not rail items; those tabs show snoozed/trashed captures with rail Inbox still highlighted; inbox row has Promote / Snooze / Trash and no checkbox/drag/due date; quick-add on Inbox only; Promote opens the existing modal; named-list / Unlisted / smart-view / + New list / rail-delete still work. HTTP driver stubs the new browser operations.

**Deliverable:** A member opens Inbox from the rail and works captures on an Inbox-first pane, with Snoozed and Trash as tabs, without a new capture model.
...
## Yoink UI story 6: Mine leftovers - Complete ✓

**Goal**: Mine is only the smart view of my assigned open tasks, grouped by list. The leftover All two-mode pile picker is gone.

**Product rules (locked, Polly 2026-09-04):**
- One story at a time. This story only.
- Remove Mine’s two-mode pile dropdown (`#mine-pile` / All-style picker on Mine).
- Mine is always the overview of my assigned open tasks, grouped by named list then Unlisted (same grouping as today’s Mine overview / Upcoming list groups).
...
- Drag, empty groups, bulk actions, Done-by-list
- Changing Today/Upcoming nesting
- Promote / Inbox changes

**Implementation:**
- Mine always uses `GET /api/tasks?filter=mine` and `groupAllTasksByPile`. Do not slice with `tasksInPile` or the list/unlisted pile APIs.
...
## Yoink UI story 8: mobile rail inside Tasks - Complete ✓

**Goal**: On a phone, the approved flat rail lives inside the Tasks tab. Bottom tabs stay Inbox | Tasks. Desktop rail does not change.

**Product rules (locked, Polly 2026-09-04):**
- One story at a time. This story only. Last of the approved sidebar frame.
- Bottom tabs stay Inbox | Tasks only. Do **not** put twelve destinations in the thumb bar.
- Inside Tasks on mobile: the same flat rail as desktop — smart views, Lists heading, named lists, Unlisted, + New list.
- Desktop rail unchanged.
- Inbox bottom tab still opens the Inbox pane (captures; Snoozed/Trash tabs on the pane). Tasks bottom tab is where you pick a smart view or named list via the rail.
- Desktop / wide layout already has the rail; do not regress it. This story is the mobile layout adaptation.
- Named-list screens, Unlisted, Today / Upcoming / Mine / Done, + New list, rail-delete, Promote sheet, Mine smart view, All retired — all stay as on trunk after story 7.
- Stay on shadcn New York in `@yoink/ui-base`. HTTP still only maps. No new domain field.
...
- Changing Today/Upcoming nesting or Mine grouping
- Reintroducing All
- New bottom tabs beyond Inbox | Tasks

**Implementation:**
- Extract `AppRailPanel` (same items + create/delete dialogs). Desktop sidebar stays `hidden md:flex`. Mobile Tasks renders the same panel (`md:hidden`).
- Mobile bottom nav stays Inbox | Tasks (`data-app-mobile-nav`). Inbox still matches `/`, `/snoozed`, `/trash`.
- Filter tabs (Today / Upcoming / Mine / Done) stay on desktop; mobile picks those views from the Tasks rail.
- Playwright uses a phone viewport (below `md`). Bottom tabs are Inbox | Tasks only. On Tasks, the rail is available and selecting an item shows that screen. Inbox bottom tab still opens the capture pane. Wide layout still shows the desktop sidebar rail. Named-list / Unlisted / smart views / + New list / rail-delete / Promote still work on mobile via Tasks + rail. HTTP driver stubs the new browser operations.

**Deliverable:** A member on a phone uses Inbox | Tasks bottom tabs, picks a smart view or named list from the rail inside Tasks, and still has the desktop sidebar on a wide layout.
...
## Yoink UI story 10: larger complete control - Complete ✓

**Goal**: Completing a task is an obvious tap on a large “done” control, not a tiny multi-select checkbox.

**Product rules (locked, issue #77):**
- One story at a time. This story only. Do not start #78 / #79.
- On task rows (all sizes, or at least mobile-first with desktop matching), replace the small multi-select-looking checkbox with a larger complete control: clear icon, larger touch target (aim ~44px).
- One tap/click still completes (and uncompletes where that already exists).
- Must not look like a multi-select checkbox. No multi-select behavior in this story.
- Pin, due date, assignee, and other row chrome stay as they are unless they block the touch target.
- Stay on shadcn New York / lucide in `@yoink/ui-base`. HTTP still only maps. No new domain field.

**Out of scope (do not implement here):**
- Swipe-to-complete (#79), multi-select, drag reorder (#78)
- Changing which tasks appear
- Mobile drawer (#76 already shipped)

**Implementation:**
- `TaskCard` uses a kit `Button` (ghost, 44px circular hit target) with lucide `Circle` / `CircleCheck` instead of the kit `Checkbox`. Role is button (`aria-pressed`), not checkbox.
- Playwright: rows show a ≥44px complete button and no checkbox; complete and uncomplete from that control still work; named-list / Unlisted / Today stay usable on desktop and phone. HTTP driver stubs the new browser operations.

**Deliverable:** A member completes and uncompletes from a large, obviously-done control on the task row, on phone and desktop.

---

## Yoink UI story 11: swipe to complete - Complete ✓

**Goal**: On a phone, swiping a task row completes it — the same outcome as the large complete control.

**Product rules (Polly lock, issue #79):**
- One story at a time. This story only. Do not start #78.
- Mobile horizontal swipe completes (same outcome as the circle control).
- Vertical scroll must not accidental-complete.
- Desktop stays tap-only.
- No swipe-to-delete. No drag in this PR.
- Direction: swipe right. Matches the existing capture swipe-right (primary processed action). Affordance is the existing `SwipeableCard` reveal (Complete / Incomplete + circle icon).
- Completing via swipe has the same outcome as the complete control (same toggle: complete if open, uncomplete if done).
- Horizontal swipe vs vertical scroll are separate axes; if drag-reorder lands later, it can own the vertical axis (or a handle).
- Desktop mouse stays tap-only. The gesture is touch-only (`useSwipe`); we do not add a mouse-drag complete path.
- The larger complete control from #77 still works.
- Stay on shadcn New York / lucide in `@yoink/ui-base`. HTTP still only maps. No new domain field.

**Out of scope (do not implement here):**
- Multi-select, drag reorder (#78)
- Changing the complete icon from #77
- Swipe-to-delete (Trash is for captures)

**Implementation:**
- `TaskCard` swipe-right is complete (was delete). Capture cards still swipe-right to trash.
- Playwright (phone viewport): swipe completes; the #77 control still completes/uncompletes; a vertical-dominant gesture plus list scroll does not complete. Desktop: mouse drag does not complete; tap still does. HTTP driver stubs the new browser operations.

**Deliverable:** A member on a phone swipes a task row to complete it, can still tap the circle control, and can still scroll the list without accidental completes.
...
**Sentry setPointerCapture on Tasks is in.** Missing-pointer `setPointerCapture` (NotFoundError) is swallowed at the one-pile grip and on `Element.prototype` so Vaul / synthetic Playwright pointerdown cannot crash the tasks surface. Named-list / Unlisted reorder still persists. Swipe-to-complete and #90 multi-slot drag are unchanged.
