...
export const migration: Migration = {
  ...
  up: (db) => {
    ...
    db.exec(`CREATE INDEX idx_memberships_user ON organization_memberships(user_id)`);

    // Index for listing members of an organization
    db.exec(`CREATE INDEX idx_memberships_org ON organization_memberships(organization_id)`);
  },
};
